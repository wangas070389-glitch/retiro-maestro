...
export default function LoginPage() {
    ...
    async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
        ...
        try {
            ...
            if (res?.error) {
                showToast("Credenciales inválidas. Por favor, verifica tu correo y contraseña.", "error");
            } else if (res?.ok) {
    ...
    return (
        <div className="min-h-screen relative flex flex-col bg-slate-50 text-slate-900 font-sans overflow-hidden selection:bg-indigo-500/30">
            ...
            <div className="relative z-10 flex-1 flex flex-col">
                ...
                <header className="w-full relative z-20 bg-white/80 backdrop-blur-md border-b border-slate-200">
                    <div className="max-w-7xl mx-auto flex justify-between items-center p-4 lg:px-8">
                        ...
                        <nav>
                            <Link href="/register" className="text-sm font-semibold text-slate-600 hover:text-indigo-600 transition-colors">
                                Crear Cuenta
                            </Link>
                        </nav>
                    </div>
                ...
                <main className="flex-1 flex flex-col items-center justify-center px-4 py-12">
                    ...
                    <div className="text-center mb-10 animate-in slide-in-from-bottom-4 duration-700">
                        ...
                        <p className="text-slate-600 max-w-sm mx-auto">
                            Ingresa a tu panel de control para optimizar tu estrategia de Modalidad 40.
                        </p>
                    </div>
                        ...
                        <div className="bg-white p-8 rounded-3xl border border-slate-200 shadow-[0_20px_40px_rgba(0,0,0,0.04)] relative overflow-hidden">
                            ...
                            <form className="space-y-5" onSubmit={handleSubmit}>
                                ...
                                <button
                                    type="submit"
                                    disabled={loading}
                                    className="w-full flex items-center justify-center py-4 px-4 rounded-xl text-md font-bold text-white bg-indigo-600 hover:bg-indigo-700 focus:ring-4 focus:ring-indigo-500/30 disabled:opacity-50 transition-all shadow-md group mt-8"
                                >
                            ...
                            <div className="mt-8 pt-6 border-t border-slate-100 text-center">
                                <span className="text-sm text-slate-500">¿No tienes una cuenta aún? </span>
                                <Link href="/register" className="text-sm font-bold text-indigo-600 hover:text-indigo-800 transition-colors">
                                    Crear cuenta libre
                                </Link>
                            </div>
                        </div>
